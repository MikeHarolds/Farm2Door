    ...
    async function fetchUsers() {
      try {
        ...
        if (res.ok) {
          ...
          setUsers((data.data || []).filter((u: User) => u.role === "driver"));
        }
      } catch (error) {
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          {users.length} driver(s) registered on the platform
        </p>
      </div>

      {users.length === 0 ? (
        <Card padding="lg">
          <EmptyState icon={<span>🚛</span>} title="No drivers registered" description="Drivers who register will appear here." />
        </Card>
      ) : (
        <Card padding="lg">
                  ...
                  <tr key={user.id} className="hover:bg-gray-50">
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center font-medium text-sm">
                          {user.name.charAt(0)}
                        </div>
                    ...
                    <td className="py-4 px-4"><StatusBadge status={user.status as any} /></td>
                    ...
                    <td className="py-4 px-4 text-right">
                      ...
                      >
                        {actionLoadingId === user.id ? "..." : user.status === "active" ? "Deactivate" : "Activate"}
                      </button>
                    </td>
                  </tr>
                ))}
